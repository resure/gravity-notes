    ...
    function createComponentFactory(registry, ensureFetched) {
       ...
       *  re-fires after the swap commits so refs exist. */
            ...
            render() {
                ...
                this.__streamingNow = Boolean(r.htmlStreaming || r.jsStreaming);
                this.__htmlStreamingNow = Boolean(r.htmlStreaming);
                return h(
                    'div',
  ...
  let CANVAS_BG_DARK = "#2e2c26";
    function createHelmetManager(doc, isStreaming) {
        ...
        function compile(node) {
            ...
            return (_vals, ctx) => {
                ...
                const streaming = Boolean(name && isStreaming(name));
    ...
    function createRuntime(doc = document) {
        ...
        function setStreaming(name, kind, on) {
            const r = registry.get(name);
            if (kind === 'html') r.htmlStreaming = Boolean(on);
            else r.jsStreaming = Boolean(on);
            let any = false;
        ...
        function dcUpdate(name, kind, content, streaming) {
            ...
            if (kind === 'html') {
                setStreaming(name, 'html', Boolean(streaming));
                updateHtml(name, content);
            } else if (kind === 'js') {
                setStreaming(name, 'js', Boolean(streaming));
