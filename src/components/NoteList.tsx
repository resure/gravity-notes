...
export function NoteList({notes, selectedId, onSelect, onCreate, onRename, onDelete}: NoteListProps) {
        ...
        {notes.length === 0 ? (
          ...
          notes.map((note) => (
            <div
              key={note.id}
              className={
                'note-list__item' + (note.id === selectedId ? ' note-list__item_selected' : '')
              }
            ...
            >
              ...
              <div className="note-list__actions" onClick={(e) => e.stopPropagation()}>
                <DropdownMenu
